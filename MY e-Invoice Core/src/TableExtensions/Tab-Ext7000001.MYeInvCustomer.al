...
tableextension 7000001 "Customer EINV" extends Customer
{
    fields
    {
        // LHDN Customer Classification
        field(50100; "Customer Type EINV"; Enum "Entity Type EINV")
        {
        ...
        // Foreign Customer
        field(50160; "Foreign Customer EINV"; Boolean)
        {
            Caption = 'Foreign Customer';
            DataClassification = CustomerContent;
            InitValue = false;
        }
    ...
    keys
    {
        key(Key50100; "Customer Type EINV")
        {
        }
    ...
    local procedure ValidateCustomerTypeFields()
    begin
        case "Customer Type EINV" of
            "Customer Type EINV"::"Foreign Individual",
            "Customer Type EINV"::"Malaysian Individual":
                begin
                    // Individual: NRIC/Passport mandatory
                    if "ID No. EINV" = '' then
                        Error('ID Number is required for Individual customers');
                end;
            "Customer Type EINV"::"Foreign Business",
            "Customer Type EINV"::"Malaysian Business":
                begin
                    // Business: TIN mandatory
                    if "TIN EINV" = '' then
                        Error('TIN is required for Business customers');
                end;
        end;
    end;
}
